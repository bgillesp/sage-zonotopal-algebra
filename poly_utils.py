...
class PolynomialFreeModuleElement(CombinatorialFreeModuleElement):
    r"""
    Very small extension of CombinatorialFreeModuleElement which adds the
    ability to convert an element back to the appropriate polynomial class.
    """
...
class PolynomialFreeModule(CombinatorialFreeModule, UniqueRepresentation,
                           Parent):
    r"""
    Class PolynomialFreeModule implements a combinatorial free module whose
    underlying elements come from a polynomial ring.  This structure is useful
    when considering the induced module or vector space structure of a
    polynomial ring coming from the multiplication of the coefficient field or
    ring.
    """
    def __init__(self, P, basis=None):
        ...
        if isinstance(basis, Monomials(self._poly_ring)):
            self._basis = basis
            self._dimn = Infinity
        ...
        elif isinstance(basis, Monomials(self._poly_ring, (0, 1))):
...
class PolyUtils:
    ...
    def pure_tensor(P, vects, indices=None):
        """
        Form a product of linear forms using vectors in the list `vects` for
        the forms and the elements of `indices` as the indices.  Items in
